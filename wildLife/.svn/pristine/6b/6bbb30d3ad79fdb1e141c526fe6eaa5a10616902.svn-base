...
(function () {
    ...
    angular.module('Education.Report').controller('CompareLiveStockProductReportController', CompareLiveStockProductReportController);

    CompareLiveStockProductReportController.$inject = [
        '$rootScope',
        '$scope',
        '$http',
        '$timeout',
    ...
    function CompareLiveStockProductReportController ($rootScope, $scope, $http, $timeout,settings,service,regionService,modal,toastr,$state,blockUI,$stateParams,utils,$translate,farmService,auService, ownershipService) {
		...
		function getAllLiveStockProduct() {
            service.getAllLiveStockProduct().then(function (data) {
				...
				if(vm.listLiveStockProduct && vm.listLiveStockProduct.length > 0){
					vm.paramDto.liveStockProductId = vm.listLiveStockProduct[0].id;
				}
            }).then(function(){
        ...
        vm.groupByItems=[
			...
			{
        		name:"Sản phẩm chăn nuôi",
        		value:"liveStockProduct"
        	},
        	{
        		name:"Xã",
        		value:"ward"
...
//        	}
		...
		function validate(){
			...
			// if(typeof vm.paramDto.liveStockProductId != typeof 0 || vm.paramDto.liveStockProductId < 0){
			// 	toastr.warning($translate.instant("report.warning_choose_lives_stock_product"),$translate.instant("toastr.warning"));
			// 	return false;
			// }
			return true;
		}
        ...
        vm.getQuantityReport=function (){
			...
			// }
			if(vm.listLiveStockProduct && vm.listLiveStockProduct.length > 0){
				vm.unitAmount = vm.listLiveStockProduct.find(el =>{
					if(el && el.id == vm.paramDto.liveStockProductId){
						return true;
					}
					return false;
				});
				vm.unitAmountView = "";
				vm.unitQuantityView="";
				if(vm.unitAmount && vm.unitAmount.unitAmount && vm.unitAmount.unitAmount.name){
					vm.unitAmountView = "("+vm.unitAmount.unitAmount.name+")";
				}
				if(vm.unitAmount && vm.unitAmount.unitQuantity && vm.unitAmount.unitQuantity.name){
					vm.unitQuantityView = "("+vm.unitAmount.unitQuantity.name+")";
				}
			}
			vm.title=$translate.instant('report.compareExportLiveStockProduct')+" ";
			...
			if(vm.paramDto.periodType==0){
				vm.title=vm.title+ vm.paramDto.fromYear+ " - "+vm.paramDto.toYear;
				...
				//'/'+vm.paramDto.toYear+" ("+$translate.instant('report.item')+")";
			}
        	...
        	vm.paramDto.groupByItems.push('liveStockProduct');			
...
//        	console.log(vm.paramDto.groupByItems.includes('district'));
            ...
            service.getLiveStockProductReport(vm.paramDto).then(function(data){
            	vm.results=data;
...
//                });
            	var configChart = {
            	    ...
            	    report: {
            	        ...
            	        "slice": {
            	            "rows": [{
        	                    "uniqueName": "liveStockProductName",
        	                    "sort": "asc",
        	                    "caption":$translate.instant('report.liveStockProduct')
            	            }],
            	            "columns": vm.rowsPivot,
            	            "measures": [{
            	                "uniqueName": "quantity",
            	                //"aggregation": "sum",
            	                "caption":$translate.instant('report.quantity')+vm.unitQuantityView
            	            }
            	            ,{
            	                "uniqueName": "amount",
            	                //"aggregation": "sum",
            	                "caption":$translate.instant('report.amount1')+vm.unitAmountView
            	            }
            	            ],
            	        },
            	        options: {
...
//        	                	 "showGrandTotals": ""
				...
				if(vm.unitAmount.unitQuantity && vm.unitAmount.unitQuantity.id && vm.unitAmount.unitAmount && vm.unitAmount.unitAmount.id){
					configChart.report.slice.measures =  [{
						"uniqueName": "quantity",
//            	                "aggregation": "sum",
						"caption":$translate.instant('report.quantity')+vm.unitQuantityView
					}
				...
				}else if(vm.unitAmount.unitQuantity && vm.unitAmount.unitQuantity.id){
					configChart.report.slice.measures =  [{
						"uniqueName": "quantity",
//            	                "aggregation": "sum",
						"caption":$translate.instant('report.quantity')+vm.unitQuantityView
				...
				}else if(vm.unitAmount.unitAmount && vm.unitAmount.unitAmount.id){
					configChart.report.slice.measures =  [{
						"uniqueName": "amount",
						//"aggregation": "sum",
						"caption":$translate.instant('report.amount1')+vm.unitAmountView
            	...
            	//Pivot here     
            	var pivot = new WebDataRocks(configChart);
            	//createChart
            	function createChart() {
        		    pivot.highcharts.getData({
						...
						data.title.text=$translate.instant('report.compareExportLiveStockProduct')+ " ";
						if(vm.wardsName!=null){
						...
						if(vm.paramDto.periodType==0){
							data.title.text=data.title.text+ vm.paramDto.fromYear+ " - "+vm.paramDto.toYear;
						}else if(vm.paramDto.periodType==1){
						...
						data.title.text=$translate.instant('report.compareExportReport')+ " ";
						if(vm.wardsName!=null){
							data.title.text=data.title.text+ vm.wardsName+ " ";
						}else if(vm.districtName!=null){
							data.title.text=data.title.text+ vm.districtName+ " ";
                ...
                if (vm.results != null && vm.results.length > 0) {
                    ...
                    for (var i = 0; i < vm.results.length; i++) {
                        ...
                        quantityArray.push(vm.results[i].amount);
