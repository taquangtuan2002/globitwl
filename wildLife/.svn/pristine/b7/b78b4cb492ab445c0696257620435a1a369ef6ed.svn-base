...
(function () {
    ...
    angular.module('Education.ReportPeriod').controller('PageTemplate16CController', PageTemplate16CController);

    PageTemplate16CController.$inject = [
        '$rootScope',
        '$scope',
        '$http',
        '$timeout',
    ...
    function PageTemplate16CController($rootScope, $scope, $http, $timeout, settings, service, modal, toastr, $state, blockUI, $stateParams, util, Uploader, FileSaver, farmService, animalService, $translate) {
        ...
        vm.getListReportFormByPage = function () {
			...
			if(changeDate()){
				service.searchByPageReportFormAnimalEgg(vm.searchObject).then(function (data) {
					if (data != null) {
        ...
        vm.getListAnimalClass = function () {
            animalService.getAll().then(function(data){
                vm.listAnimal16C=data.filter(x=>x.reproductionForm==2);
                animalService.getListAnimalClass().then(function (data) {
                    ...
                    data.forEach(item => {
                        ...
                        var check = vm.listAnimal16C.filter(x=>x.animalClass==item);
                ...
                animalService.getListAnimalOrdo(vm.searchDto.animalClass).then(function (data) {
                    ...
                    data.forEach(item => {
                        ...
                        var check = vm.listAnimal16C.filter(x=>x.ordo==item);
                        if(check.length>0||item==""){
                ...
                animalService.getListAnimalFamily(vm.searchDto.animalClass, vm.searchDto.ordo).then(function (data) {
                    ...
                    data.forEach(item => {
                        ...
                        var check = vm.listAnimal16C.filter(x=>x.family==item);
                        if(check.length>0||item==""){
        ...
        vm.getListAnimalFamily = function (callSearchFunction) {
            animalService.getAll().then(function(data1){
                vm.listAnimal16C=data1.filter(x=>x.reproductionForm==2);
                animalService.getListAnimalFamily(vm.searchDto.animalClass, vm.searchDto.ordo).then(function (data) {
                    vm.listAnimalFamily = [];
                    vm.family = null;
                    data.forEach(item => {
        ...
        vm.searchByCode = function(){
            ...
            animalService.getPageSearchAnimal(vm.searchDto, 1, 1000000).then(function (data) {
                vm.animals = data.content.filter(x=>x.reproductionForm==2);
            });
        }
		// animalService.getAll().then(function (data) {
		// 	vm.animals = data;
        ...
        $scope.getDataDistrictAndWard = function () {
			...
			if(changeDate()){
				service.exportReportFormAnimalEgg16C(searchDto)
                .then(function (data) {//is important that the data was returned as Aray Buffer
                	var file = new Blob([data], {
                		type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
                    });
                    FileSaver.saveAs(file, "DuLieuMau16C.xls");
                }, function (fail) {
        ...
        vm.delete16CById=function(id){
            vm.modalConfirmDelete16C = null;
            vm.modalConfirmDelete16C = modal.open({
                animation: true,
                templateUrl: 'confirm_delete16C.html',
                scope: $scope,
                backdrop: 'static',
                keyboard: false,
                size: 'md'
            });
            vm.modalConfirmDelete16C.result.then(function (confirm) {
                if (confirm == 'yes') {
                    // check editable
                    //debugger
                    service.getReportFormAnimalEggById(id).then(function(res){
                        ...
                        service.getAdministrativeUnitDtoByLoginUser().then(function (data){
                            ...
                            if((todayYear==dateReportYear&&(todayQuarter === periodQuarter))||vm.isRoleAdmin==true){
                                service.deleteReportFormAnimalEgg(id).then(function (r) {
                                    if(r){
                                    ...
                                    service.getUserAdminstrativeEditTableUnit(vm.adminstrativeUnits[i].parentId!=null?vm.adminstrativeUnits[i].parentId:vm.adminstrativeUnits[i].id).then(function (data1){
                                        ...
                                        if(vm.userEnableEdit && vm.userEnableEdit.length>0){
                                            ...
                                            if(editable==true){
                                                ...
                                                service.deleteReportFormAnimalEgg(id).then(function (r1) {
                                                    if(r1){
                                                        toastr.info("Xóa bản ghi thành công", "Thông báo");
                                                        vm.search();
                                                    }else{
