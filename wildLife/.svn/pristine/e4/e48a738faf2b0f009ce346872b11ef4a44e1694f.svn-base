...
(function () {
    ...
    angular.module('Education.Report').controller('ReportImportAnimalController', ReportImportAnimalController);

    ReportImportAnimalController.$inject = [
        '$rootScope',
        '$scope',
        '$http',
        '$timeout',
    ...
    function ReportImportAnimalController ($rootScope, $scope, $http, $timeout,settings,service,regionService,modal,toastr,$state,blockUI,$stateParams,utils,$translate,farmService,auService,animalService,FileSaver) {
        ...
        var today = new Date();
        // vm.toDate = new Date();
        vm.fromDate = new Date('2021-01-01T17:00:00.000Z');
        vm.fromDate.setFullYear(today.getFullYear());
        vm.fromDate.setMonth(today.getMonth());
        vm.fromDate.setDate(1);
		...
		vm.reportCurrentStatusByFamily = function(){
            ...
            service.getReportImportAnimal(searchDto).then(function(data){
                vm.results = data;
            });
		}
        ...
        vm.showDetail16 = function(item){
            ...
            service.getAllForm16ByFarm(item.farmId).then(function (data) {
                ...
                for(let i=0;i<vm.listReportForm16A.length;i++){
                    ...
                    if(vm.listReportForm16A[i].animal.id == item.animalId && date1 === date2){
