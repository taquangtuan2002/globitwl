(function () {
    ...
    angular.module('Education.TotalReport').service('TotalReportService', TotalReportService);

    TotalReportService.$inject = [
        '$http',
        '$q',
        '$filter',
        'settings',
    ...
    function TotalReportService($http, $q, $filter, settings, utils) {
        var self = this;
      ...
      //  self.getsearchImportAnimalDto=getsearchImportAnimalDto;
		...
		function getStream(reportDto){
            console.log("RUNNING");
            var deferred = $q.defer();

            $http({
                url:settings.api.baseUrl + settings.api.apiPrefix  +'file/exportToTalReport' ,
