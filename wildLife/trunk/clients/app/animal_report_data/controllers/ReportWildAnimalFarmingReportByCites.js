...
(function () {
  ...
  angular
    ...
    .controller(
      "ReportWildAnimalFarmingReportByCites",
      ReportWildAnimalFarmingReportByCites
    );

  ReportWildAnimalFarmingReportByCites.$inject = [
    "$rootScope",
    "$scope",
    "$http",
    "$timeout",
  ...
  function ReportWildAnimalFarmingReportByCites(
    $rootScope,
    $scope,
    $http,
    $timeout,
    ...
    vm.getReportNumberAnimalsAndNumberFarmsAccordingToTheRedBook = function () {
      ...
      vm.title = "BC 07: Tổng hợp về các loài động vật hoang dã theo NĐ06/NĐ84" + " ";
      ...
      service
        .wildAnimalFarmingReportByClassification07(vm.paramDto)
        .then(function (data) {
          console.log(data)
          vm.results = data;
          ...
          vm.rowName = [
            {
              caption: "Tên thông thường",
              uniqueName: "animalName",
            },
            {
              caption: "Tên khoa học",
              uniqueName: "scienceName",
            },
          ];
          vm.rows = [
            // {
          ...
          // vm.grid = {
          //   title:
          //     "Báo cáo tổng hợp nuôi động vật hoang dã theo phụ lục cites",
          //   showTotals: "rows",
          //   showGrandTotals: "rows",
          // };
          vm.grid = {
            // type: "classic",
            title: "Báo cáo tổng hợp nuôi động vật hoang dã theo NĐ 06-2019",
            title: vm.title,
            type: "classic",
            showTotals: "off",
            showGrandTotals: "off",
          ...
          //   ];
          // }
          if (vm.isShowProvinceData == true) {
            vm.rows.push({
              caption: "Tỉnh",
              uniqueName: "provinceName",
            });
          }
          if (vm.isShowDistricData == true) {
            vm.rows.push({
              caption: "Huyện",
              uniqueName: "districtName",
            });
          }
          if (vm.isShowWardData == true) {
            vm.rows.push({
              caption: "Xã",
              uniqueName: "wardName",
            });
          ...
          vm.rows.push(
            ...
            // },
            // {
            //   caption: "Số cơ sở nuôi",
            //   uniqueName: "numberOfFam",
            //   aggregation: "sum",
            // },
            // {
            //   caption: "Số cơ sở đã đăng kí",
            //   uniqueName: "numberOfFarmStatus",
            //   aggregation: "sum",
            // },
            ...
            pivot.highcharts.getData({}, function (data) {
              ...
              data.series.map((s) => {
                ...
                data.xAxis.categories.map((c, i) => {
                  ...
                  if (s.name == "Số cơ sở nuôi") {
                    dataOfFam.push(slice);
                  }
                  if (s.name == "Số cơ sở đã đăng kí") {
              ...
              const getDataChart = (e) => {
                ...
                if (e.name == "Số cơ sở nuôi") {
                  return dataOfFam;
                }
                if (e.name == "Số cơ sở đã đăng kí") {
                  return dataOfAnimal;
                }
                return [];
              };
