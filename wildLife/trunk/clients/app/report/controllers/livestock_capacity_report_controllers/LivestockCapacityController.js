...
(function () {
    ...
    angular.module('Education.Report').controller('LivestockCapacityController', LivestockCapacityController);

    LivestockCapacityController.$inject = [
        '$rootScope',
        '$scope',
        '$http',
        '$timeout',
    ...
    function LivestockCapacityController ($rootScope, $scope, $http, $timeout,settings,service,regionService,modal,toastr,$state,blockUI,$stateParams,utils,$translate,farmService,auService) {
        ...
        vm.paramDto.densityCoefficient=7;//Mật độ chăn nuôi dự kiến
...
//        vm.getAllCity();
        ...
        vm.getQuantityReport=function (){
			...
			vm.title=$translate.instant('report.livestock_capacity')+ " ";
			if(vm.checkStatus && vm.checkStatus==true){
	        	vm.paramDto.status=null;
	        }
	        else{
	        	vm.paramDto.status=1;
	        }
			...
			else if(vm.paramDto.provinceId==null){
				vm.rowsPivot=[{
                    "uniqueName": "provinceName",
                    "sort": "asc",
                    "caption":$translate.instant('Tỉnh')
    	            }];
			}
			else if(vm.paramDto.districtId==null){
				vm.rowsPivot=[{
                    "uniqueName": "districtName",
                    "sort": "asc",
                    "caption":$translate.instant('Huyện')
    	            }];
			}else if(vm.paramDto.wardId==null){				
				vm.rowsPivot=[{
                    "uniqueName": "wardName",
                    "sort": "asc",
                    "caption":$translate.instant('Xã')
    	            }];
			}
			else{
				vm.rowsPivot=[{
            ...
            service.getLivestockCapacityReport(vm.paramDto).then(function(data){
            	vm.results=data;
            	...
            	var pivot = new WebDataRocks({
            	    ...
            	    report: {
            	        ...
            	        "slice": {
            	            ...
            	            "measures": [
            	            	{
	            	                "uniqueName": "totalAcreage",
	//            	                "aggregation": "sum",
	            	                "caption":$translate.instant('report.breedingArea')
	            	            },{
	            	                "uniqueName": "livestockCapacityByTotal",
	            	                //"aggregation": "sum",
	            	                "caption":$translate.instant('report.livestockCapacityByTotal')
	            	            }
	            	            ,{
	            	                "uniqueName": "lodgingAcreage",
	            	                //"aggregation": "sum",
	            	                "caption":$translate.instant('report.lodgingAcreage')
	            	            },{
	            	                "uniqueName": "livestockCapacityByLodging",
	            	                //"aggregation": "sum",
	            	                "caption":$translate.instant('report.livestockCapacityByLodging')
	            	            }
            	            ],
            	            "sorting": {
            	                "row": {
            	...
            	function createChart() {
        		    pivot.highcharts.getData({
						...
						data.title.text=$translate.instant('report.livestock_capacity')+" ";
						if(vm.wardsName!=null){
