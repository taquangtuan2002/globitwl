...
(function () {
    ...
    angular.module('Education.Report').controller('ReportForestProductListController', ReportForestProductListController);

    ReportForestProductListController.$inject = [
        '$rootScope',
        '$scope',
        '$http',
        '$timeout',
    ...
    function ReportForestProductListController ($rootScope, $scope, $http, $timeout,settings,service,regionService,modal,toastr,$state,blockUI,$stateParams,utils,$translate,farmService,auService,animalService,FileSaver, forestProductService) {
		...
		vm.reportCurrentStatusByFamily = function(){
            ...
            service.getReportForestProductList(searchDto).then(function(data){
                vm.results = data;
            });
		}
        ...
        vm.forestProduct={};
        vm.detailForestProduct = {};
        vm.showDetailForestProductPopup = function(item){
            // debugger
            var itemId = item.rowNumber;
            vm.forestProduct={};
            vm.detailForestProduct = {};
            vm.allOriginal = [];

            //service lấy forestProductList
            service.getForestProductById(itemId).then(function(data){
                vm.forestProduct = data;
                vm.forestProduct.isNew = false;
                vm.forestProduct.isCheckFarm = true;
                vm.listForesProductDetail = data.details;
            });
            // service lấy nguồn gốc lâm sản
            forestProductService.getAllOriginal().then(function (data) {
                vm.allOriginal = data;
            });
            vm.detailForestProductPopup = modal.open({
                animation: true,
                templateUrl: 'detail_forest_product.html',
                scope: $scope,
                backdrop: 'static',
                keyboard: false,
        ...
        vm.printFileWord=function(){
            forestProductService.saveForestProductList(vm.forestProduct).then(function (data) {
                if (data && data.id) {
